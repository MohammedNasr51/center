...
async function getDb() {
  if (!MONGODB_URI) {
    throw new Error(
      "MONGODB_URI is missing. Add it in .env locally and in Deno Deploy environment variables.",
    );
  }
  if (!clientPromise) {
...
function cleanRecord(record: Record<string, unknown>) {
  const { _id, id, createdAt, updatedAt, centerId, type, ...rest } =
    record || {};
  return rest;
}
...
app.get("/", (c) =>
  c.json({ ok: true, app: "Vision Center Backend", docs: "/api/health" }),
);
...
app.put("/api/state", async (c) => {
  ...
  if (!body || typeof body !== "object")
    return jsonError(c, "Invalid JSON body");
...
app.get("/api/:collection", async (c) => {
  ...
  if (!allowedCollections.has(collection))
    return jsonError(c, "Collection is not allowed", 404);
  const db = await getDb();
  const centerId = getCenterId(c);
  const rows = await db
    .collection("records")
...
app.get("/api/:collection/:id", async (c) => {
  ...
  const doc = await db
    .collection("records")
    .findOne({ _id: objectId, centerId, type: collection });
...
app.put("/api/:collection/:id", async (c) => {
  ...
  const centerId = String(
    body.centerId || c.req.query("centerId") || "vision-main",
  ).trim();
  const payload = cleanRecord(body.record || body);
  const now = new Date();
...
app.delete("/api/:collection/:id", async (c) => {
  ...
  const result = await db
    .collection("records")
    .deleteOne({ _id: objectId, centerId, type: collection });
  return c.json({
    ok: true,
    centerId,
    collection,
